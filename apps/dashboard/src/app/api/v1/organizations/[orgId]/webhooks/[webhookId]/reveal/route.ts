...
/**
 * POST: Reveal a webhook's signing secret
...
export const POST = withRateLimit(
  async (req: Request, { params }: { params: Promise<{ orgId: string; webhookId: string }> }) => {
    try {
      ...
      // 4. Return the full signing secret
      return NextResponse.json({ signingSecret: decrypt(webhook.signingSecret) });
    } catch (error: unknown) {
      console.error('[Org Webhook Reveal] Error:', error);
